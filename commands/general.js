...
smd({pattern: "gpt",desc: "chat with an AI",category: "ai",use: '<Hii, Suhail Tech Info>',filename: __filename,},
  ...
  if (!text) return msg.reply(`Hey there! ${msg.pushName}. How are you doing these days?`); return msg.send(await aiResponce(msg, "gpt",  text));
    });
//---------------------------------------------------------------------------
smd({pattern: "dalle",alias : ['dall','dall-e'],desc: "chat with an AI",category: "ai", use: '<Hii, Suhail Tech Info>',filename: __filename,},
    async(Aviator, msg,text) =>{
  if ( Config.OPENAI_API_KEY=='' ||  !Config.OPENAI_API_KEY.startsWith('sk') ) return msg.reply('```You Dont Have OPENAI API KEY \nPlease Create OPEN API KEY from Given Link \nhttps://platform.openai.com/account/api-keys\nAnd Set Key in Heroku OPENAI_API_KEY Var```')
  if (!text) return msg.reply(`*Give Me A Query To Get Dall-E Reponce ?*`); return await Aviator.bot.sendMessage(msg.chat,{image:{url:await aiResponce(msg, "dalle", text)},caption: Config.caption} )
  }
)
//---------------------------------------------------------------------------
...
async(Aviator, msg, text) => {
  ...
  try{
    ...
    let anu = await TelegraPh(media);try { await fs.unlinkSync(media); } catch (error) { }          
    let res = await aiResponce(msg, "rmbg",  anu); res  ? await msg.send(res, {caption : Config.caption} , "image",msg) : await msg.send("*_Request not be preceed!!_*")
