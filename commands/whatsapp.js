...
///============================================================================================
smd({ pattern: "ptv", desc: "send ptv Message of video", category: "whatsapp",filename: __filename},async(Lucifer, msg, text,{cmdName , isCreator}) => {
   ...
   return await forwardMessage(msg.chat, Lucifer, msg, cmdName )
})////------------------------------------------------------------
smd({pattern: "#", desc: "Save whatsapp status",category: "whatsapp",filename: __filename},async(Lucifer, msg, text,{cmdName , isCreator}) => {
   if(!msg.quoted) return await msg.send("*Uhh Please, reply to whatsapp status*")
   let Lucifer_MD_num = await Lucifer.bot.decodeJid(msg.user)
   return await forwardMessage(sᴜʜᴀɪʟ_ᴍᴅ_num, Lucifer, msg, cmdName )
})//--------------------------------------------------------------------
smd({pattern: "save",desc: "Save Message to log number",category: "whatsapp",filename: __filename},async(Lucifer, msg, text,{cmdName , isCreator}) => {
   if(!isCreator) return await msg.send(tlang().owner)
   if(!msg.quoted) return await msg.send("*Uhh Please, reply to a Message*")
   let Lucifer_MD_num = await Lucifer.bot.decodeJid(msg.user)
   return await forwardMessage(sᴜʜᴀɪʟ_ᴍᴅ_num, Lucifer, msg, cmdName )
})///================================================================================
smd({ on: "text" }, async (Lucifer,msg,text)=> {
   if(msg.quoted && msg.text.toLowerCase().includes("send") ){
       let Lucifer_MD_num = await Suhail.bot.decodeJid(msg.user)
...
async(Lucifer, msg,text) => {
   ...
   const Lucifer_MD_num = await Suhail.bot.decodeJid(msg.user)
