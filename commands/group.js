...
async(Lucifer, msg, text , { cmdName ,isCreator}) => {
...
async(Lucifer, msg, text,{ isCreator }) => {
    ...
    try {
        await Lucifer.bot.groupUpdateDescription(msg.chat, text);
...
async(Lucifer, msg, text,{ isCreator }) => {
    ...
    try {
        await Lucifer.bot.groupUpdateSubject(msg.chat, text)
...
async(Lucifer, citel, text,{ isCreator }) => {         
...
`;
            ...
            Lucifer.bot.sendMessage(msg.chat, buttonMessage, { quoted: msg });
    ...
    userq.level + 1
            ...
            try {
                ppuser = await Lucifer.bot.profilePictureUrl(msg.sender, "image");
            } catch {
                ppuser = THUMB_IMAGE;
            }
    ...
    smd({
        ...
        async(Lucifer, msg, text ,{ isCreator }) => {	
            ...
            try {
                ...
                await Lucifer.bot.groupParticipantsUpdate(msg.chat, [users], "promote");
                await msg.send(`*User promoted Succesfully!*`)
                return await Lucifer.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
            } catch(e) {
                console.log("Promote error : " , e )
                await Lucifer.bot.sendMessage(msg.chat, { react: { text: '❌', key: msg.key }});
...
//if (!isCreator) return msg.reply("```Only My Owner Can Use This Command```")
    if (!msg.isGroup) return msg.reply(tlang().group);
    const groupAdmins = await getAdmin(Lucifer.bot, msg)
    const botNumber = await Lucifer.bot.decodeJid(Lucifer.bot.user.id)
    const isBotAdmins = msg.isGroup ? groupAdmins.includes(botNumber) : false;
    const isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) : false;
    if (!isBotAdmins) return await msg.reply(`*_I'm Not Admin In This Group, Idiot_*`); 
    if (!isAdmins) return msg.reply(tlang().admin);
    ...
    try {
        let users = msg.quoted ? msg.quoted.sender : msg.mentionedJid[0] ? msg.mentionedJid[0] : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
        if (!users) return await msg.send("*_Uhh Dear, Reply/Mention to an User_*");
        await Lucifer.bot.groupParticipantsUpdate(msg.chat, [users], "demote");
...
smd({
	...
	//if (!isCreator) return msg.reply("*_Only My Owner Can Use This Command_*")
            ...
            try {
                ...
                await Lucifer.bot.groupParticipantsUpdate(msg.chat, [users], "remove");
...
cmd({
        ...
        async(Lucifer.bot, msg, text) => {
            ...
            let mee = await Lucifer.bot.downloadAndSaveMediaMessage(msg.quoted)
            let mem = await TelegraPh(mee)
            let meme = `https://api.memegen.link/images/custom/-/${text}.png?background=${mem}`
            let memek = await Lucifer.bot.sendImageAsSticker(msg.chat, meme, msg, { packname: msg.pushName, author: 'Secktor' })
...
return await Lucifer.bot.sendMessage(msg.chat,{image:{url : pp} , caption: ginfos } , {quoted:msg })
}
else
{ 
...
/*
            ...
            await Lucifer.bot.updateProfilePicture(msg.chat, {
                    url: media,
                })
...
async(Lucifer, msg, text , {isCreator}) => {
    if (!msg.isGroup) return msg.reply(tlang().group);
    const groupMetadata = msg.isGroup ? await Lucifer.bot.groupMetadata(msg.chat).catch((e) => {}) : "";
    const participants = msg.isGroup ? await groupMetadata.participants : "";
    const groupAdmins = participants.filter(p => p.admin)
    const isAdmins = msg.isGroup ? groupAdmins.includes(msg.sender) : false;
    if (!isAdmins ) return msg.reply(tlang().admin);
...
└───────────
`.trim()
return await Lucifer.bot.sendMessage(msg.chat,{text : tag ,mentions: [msg.sender, ...groupAdmins.map(v => v.id) ,]} ,)
    ...
    smd({
        ...
        async(Lucifer, msg, text,{ isCreator }) => {
            if (!isCreator) return msg.reply(tlang().owner)
            n = await Lucifer.bot.groupFetchAllParticipating();
...
return await msg.reply(res);
	...
	let getGroups = await Lucifer.bot.groupFetchAllParticipating();
...
smd({
        ...
        async(Lucifer, msg, text,{ isCreator }) => {
            if (!msg.isGroup && isCreator) { 
                ...
                return await Lucifer.bot.sendMessage(msg.chat, { delete: key })
            }
            if (!msg.quoted.isBot ) {
                if (!msg.isGroup) return msg.reply(tlang().group)
                const groupAdmins = await getAdmin(Lucifer.bot, msg)
...
smd({ on: "text" }, async(Lucifer, msg) => {
	    ...
	    if (hasLeveledUp) 
	    {
		    ...
		    {
			    await Lucifer.bot.sendMessage(msg.chat, { image: {  url: await botpic() },
			caption: `
╔════◇
║ *Wow,Someone just*
║ *leveled Up huh⭐*
